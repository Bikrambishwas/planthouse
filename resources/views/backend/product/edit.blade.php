...
@section('content')
    ...
    <form action="{{ route('admin.product.update', $row->id) }}" id="updateproduct" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row mt-2">
            <div class="col-md-9">
                <div class="card">
                                ...
                                <div class="media-holder mb-2">
                                        ...
                                        @foreach ($galleri_images as $image)
                                            <div class="media-image-holder position-relative" style="width:150px; height:100px; border:1px solid #ccc; border-radius:8px; display:flex; justify-content:center; align-items:center;">
                                                <img src="{{ asset('uploads/product_images/' . $image) }}" alt="Preview" style="width:100%; height:100%; object-fit:cover;">
                                                <span class="inbtn inbtndelete" data-imgname="{{ $image }}" style="position:absolute; top:-10px; right:-10px; background:#f77f7fcf; color:white; padding:2px; cursor:pointer;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="type" id="product_type" class="form-select">
                                ...
                                <option value="simple" {{ $row->type == 'simple' ? 'selected' : '' }}>Simple Product</option>
                                <option value="variable" {{ $row->type == 'variable' ? 'selected' : '' }}>Variable Product</option>
                            </select>
                        </div>
                            ...
                            <div id="simple_product_section" class="row" style="display: none;">
                                <div class="mb-3 col-md-6">
                                    ...
                                    <input type="number" class="form-control" name="marked_price" value="{{ $row->marked_price }}" placeholder="Enter MRP">
                                </div>
                                <div class="mb-3 col-md-6">
                                    <label class="form-label">Selling Price</label>
                                    <input type="number" class="form-control" name="selling_price" value="{{ $row->selling_price }}" placeholder="Enter Selling Price">
                                </div>
                            </div>
                                ...
                                <div class="card main-card" style="box-shadow: rgba(0, 0, 0, 0.02) 0px 1px 3px 0px, rgba(27, 31, 35, 0.15) 0px 0px 0px 1px !important;">
                                    <div class="card-body main-card-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <label>Title</label>
                                                <input type="text" class="form-control" name="plans[{{ $ind }}][title]" value="{{ $item['title'] }}" id="">
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label">Marked Price</label>
                                                <input type="text" class="form-control" name="plans[{{ $ind }}][marked_price]" value="{{ $item['marked_price'] }}" placeholder="Enter Marked Price">
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label">Selling Price</label>
                                                <input type="text" class="form-control" name="plans[{{ $ind }}][selling_price]" value="{{ $item['selling_price'] }}" placeholder="Enter Selling Price">
                                            </div>

                                            <div class="col-md-6">
                                                <label class="form-label">Image</label>
                                                <input type="file" class="form-control" name="plans[{{ $ind }}][image]" placeholder="Rs 1200000">
                                            </div>
                                        </div>
                                        <div class="row mt-2">
                                            ...
                                            <div class="facilities">
                                                            ...
                                                            <div class="col-md-11">
                                                                <div class="row">
                                                                    <div class="mb-2 col-md-6">
                                                                        <label class="form-label">Title</label>
                                                                        <input type="text" class="form-control" name="plans[{{ $ind }}][facilities][{{ $in }}][title]" value="{{ $pitem['title'] }}"
                                                                        placeholder="eg. Social Media Handling">
                                                                    </div>
                                                                    <div class="mb-2 col-md-6">
                                                                        <label class="form-label">Marked Price</label>
                                                                        <input type="text" class="form-control" name="plans[{{ $ind }}][facilities][{{ $in }}][marked_price]" value="{{ $pitem['marked_price'] }}"
                                                                        placeholder="eg. Social Media Handling">
                                                                    </div>
                                                                    <div class="mb-2 col-md-6">
                                                                        <label class="form-label">Selling Price</label>
                                                                        <input type="text" class="form-control" name="plans[{{ $ind }}][facilities][{{ $in }}][selling_price]" value="{{ $pitem['selling_price'] }}"
                                                                        placeholder="eg. Social Media Handling">
                                                                    </div>
                                                                    <div class="mb-2 col-md-6">
                                                                        <label class="form-label">Image</label>
                                                                        <input type="file" class="form-control" name="plans[{{ $ind }}][facilities][{{ $in }}][image]"
                                                                        placeholder="eg. Social Media Handling">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                ...
                                                <div class="facility-row row">
                                                    ...
                                                    <div class="col-md-1 d-flex align-items-center" style="margin-top: 20px; gap:5px">
                                                        <a onclick="addFacility(this)" class="dropdown-item add-facility-btn ad-btn"  data-fac-count="0">
                                                            <i class="fa-solid fa-plus"></i>
                                                        </a>
                                                        <a onclick="removeFacility(this)" class="dropdown-item rem-btn">
                                                            <i class="fa-solid fa-xmark"></i>
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-2">
                                <div class="dashed-box">
                                    <img src="{{ asset('uploads/product_images/' . $row->image) }}" id="editshowimage"
                                        alt="">
                                </div>
                                <label class="form-label">Image</label>
                         ...
                         <div class="row">
                            <ul class="accounts">
                                @foreach ($categories as $index => $item)
                                    <li class="cat-menu"><label><input type="radio" name="category_id" {{ $row->category_id == $item->id ? 'checked' : '' }}> {{ $item->title }}</label>
                                        <ul class="subaccounts">
                                            @foreach ($item->children as $child)
                                                <li class="cat-menu"><label><input type="radio" name="category_id" {{ $row->category_id == $child->id ? 'checked' : '' }}> {{ $child->title }}</label></li>
                                            @endforeach
                                        </ul>
                                    </li>
                                @endforeach
